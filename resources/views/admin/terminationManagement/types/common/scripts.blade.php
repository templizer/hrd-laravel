...
<script>
    ...
    function openModal(id = null,branch_id = null, title = '') {
        ...
        const form = document.getElementById('terminationTypeForm');
        ...
        if (id) {
            // Edit mode
            form.action = `{{ url('admin/termination-types') }}/${id}`;
            methodField.innerHTML = '@method("PUT")';
            modalTitle.textContent = '{{ __("index.edit") }} {{ __("index.termination_types") }}';
            ...
            // Create mode
            form.action = '{{ route("admin.termination-types.store") }}';
            modalTitle.textContent = '{{ __("index.add_termination_types") }}';
    ...
    document.getElementById('terminationTypeForm').addEventListener('submit', function(e) {
        e.preventDefault();
        this.submit();
    });
